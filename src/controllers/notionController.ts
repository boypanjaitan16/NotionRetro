...
export async function createDatabase(req: Request, res: Response) {
	...
	try {
		...
		if (collectionId) {
			...
			return res.redirect(
				`/collections/${collectionId}/todos?databaseCreated=true&dbId=${database.id}&t=${Date.now()}`,
			);
		}
		...
		if (collectionId) {
			return res.redirect(
				`/collections/${collectionId}/todos?error=create_database_failed`,
			);
		}
...
export async function exportToNotion(req: Request, res: Response) {
	...
	try {
		...
		if (todos.length === 0) {
			return res.redirect(
				`/collections/${collectionId}/todos?error=no_todos_to_export`,
			);
		}
		...
		return res.redirect(
			`/collections/${collectionId}/todos?error=${encodeURIComponent(errorMessage)}`,
		);
	}
}
...
export async function createPage(req: Request, res: Response) {
	...
	try {
		...
		if (collectionId) {
			// Add a timestamp to ensure the page is refreshed and not cached
			return res.redirect(
				`/collections/${collectionId}/todos?pageCreated=true&pageId=${page.id}&t=${Date.now()}`,
			);
		}
		...
		if (collectionId) {
			return res.redirect(
				`/collections/${collectionId}/todos?error=create_page_failed`,
			);
		}
...
export async function exportToPage(req: Request, res: Response) {
	...
	try {
		...
		// Redirect back to the todos page with a success message
		return res.redirect(
			`/collections/${collectionId}/todos?success=exported_to_page`,
		);
	} catch (error: any) {
