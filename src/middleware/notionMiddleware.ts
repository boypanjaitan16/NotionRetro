...
import { validateNotionToken, isTokenExpired } from '../services/notionService';
import { updateNotionToken } from '../services/userService';

export async function validateNotionAuth(req: Request, res: Response, next: NextFunction) {
  // @ts-ignore
  const user = req.user;
  ...
  try {
    ...
    if (user.notionTokenExpiresAt && await isTokenExpired(user.notionTokenExpiresAt)) {
