...
export async function getNotionAuthUrl(clientId: string, redirectUri: string): Promise<string> {
...
export async function exchangeCodeForToken(code: string, clientId: string, clientSecret: string, redirectUri: string): Promise<NotionTokenResponse> {
...
export async function validateNotionToken(accessToken: string): Promise<boolean> {
  try {
    ...
    if (error.response && (error.response.status === 401 || error.response.status === 403)) {
      console.log('Notion token validation failed: Token is invalid or expired');
      return false;
    }
...
export async function isTokenExpired(expiresAt: Date | null | undefined): Promise<boolean> {
...
export async function exportCollectionToNotion(accessToken: string, databaseId: string, todos: any[]) {
  const url = `${NOTION_API_BASE_URL}/pages`;
  const results = [];
  
  try {
    console.log(`Starting export to Notion database: ${databaseId} with ${todos.length} todos`);
    ...
    console.log(`Database properties:`, Object.keys(properties).map(key => ({
      name: key,
      type: properties[key].type
    })));
    
    // Find title property and checkbox property
    let titleProperty = Object.keys(properties).find(key => 
      properties[key].type === 'title'
    ) || 'Name';
    
    let checkboxProperty = Object.keys(properties).find(key => 
      properties[key].type === 'checkbox'
    ) || 'Completed';
    
    console.log(`Using title property: ${titleProperty}, checkbox property: ${checkboxProperty}`);
    
    for (const todo of todos) {
      // Ensure boolean type for completed
      const isCompleted = todo.completed === 1 || todo.completed === true;
      
      console.log(`Exporting todo: "${todo.title}", completed: ${isCompleted} (original value: ${todo.completed})`);
      ...
      console.log(`Request body for todo "${todo.title}":`, JSON.stringify(requestBody, null, 2));
        ...
        children: [
          {
            ...
            type: "heading_2",
            heading_2: {
              rich_text: [{ type: "text", text: { content: "Todo Collection" } }]
            }
          },
          {
            object: "block",
            type: "paragraph",
            paragraph: {
              rich_text: [{ type: "text", text: { content: "Exported from NotionRetro" } }]
            }
          }
        ]
      },
...
export async function exportCollectionToNotionPage(accessToken: string, pageId: string, todos: any[]) {
  try {
    console.log(`Starting export to Notion page: ${pageId} with ${todos.length} todos`);
