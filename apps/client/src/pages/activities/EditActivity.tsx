...
import {
	type UpdateActivityValues,
	updateActivitySchema,
} from "@/schemas/activity.schema";

const EditActivityPage = () => {
	const { activityId, id } = useParams<{ activityId: string; id: string }>();
	const getActivity = useActivity(asInt(activityId));
	const activity = getActivity.data?.data;

	const navigate = useNavigate();

	const updateActivity = useUpdateActivity();

	const form = useForm<UpdateActivityValues>({
		mode: "uncontrolled",
		initialValues: activity,
		validate: zod4Resolver(updateActivitySchema),
	});

	const handleUpdate = async (data: UpdateActivityValues) => {
		updateActivity
			.mutateAsync({
				id: asInt(activityId),
				data,
	...
	return (
		<div className="py-5 md:py-12">
			<div>
				...
				<h2 className="text-2xl">Update Activity</h2>
			</div>
